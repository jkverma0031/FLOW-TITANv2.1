...
class Plan(BaseModel):
    ...
    # Renamed from 'dsl'/'ast' to match Planner/Test usage
    dsl_text: Optional[str] = None
    parsed_ast: Optional[Any] = None  # parser-specific AST
    ...
    def to_summary(self) -> Dict[str, Any]:
        dsl_preview = self.dsl_text if self.dsl_text else ""
        if len(dsl_preview) > 512:
            dsl_preview = dsl_preview[:512] + "..."
            
        return {
            "id": self.id,
            "dsl_snippet": dsl_preview,
            "status": self.status,
    ...
    def canonical_hash(self) -> Optional[str]:
        if not self.cfg:
            return None
        return self.cfg.canonical_hash()  # Assuming CFG has this method, else None